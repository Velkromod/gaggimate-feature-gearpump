...
void PressureController::filterSetpoint(float rawSetpoint) {
    if (!_setpointFilterInitialized) initSetpointFilter();
...
void PressureController::initSetpointFilter(float val) {
    ...
    if (val != 0.0f) _filteredSetpoint = val;
...
void PressureController::update(ControlMode mode) {
    ...
    if ((mode == ControlMode::FLOW || mode == ControlMode::PRESSURE) && *_rawPressureSetpoint > 0.0f &&
        ...
        *_ctrlOutput = std::min(flowOutput, pressureOutput);
        if (flowOutput < pressureOutput) _errorIntegral = 0.0f;
...
float PressureController::getPumpDutyCycleForFlowRate() const {
    const float availableFlow = getAvailableFlow();
    if (availableFlow <= 0.0f) return 0.0f;
...
void PressureController::virtualScale() {
    ...
    float effectiveCompliance = 3.0f / fmax(0.2f, _filteredPressureSensor);
    ...
    if (_waterThroughPuckFlowRate > 0.0f && *_valveStatus == 1 && _filteredPressureSensor > 0.8f) {
        ...
        if (_puckCounter <= 1) _lastPuckConductance = _puckConductance;
        ...
        if (_puckConductanceDerivative < -0.5f && float(_puckCounter) * _dt > 1.0f) _puckState[0] = true;
...
float PressureController::getPressureFeedforwardDynamicUnfiltered(float pressureRef, float pressureRefDerivative, float absError) const {
    ...
    if (risingRefDerivative <= 0.0f) return 0.0f;
    ...
    float errorWeight = smoothstep(_ffErrorGateStartBar, _ffErrorGateFullBar, absError);

    float ff = _feedforwardRampGain * requiredPercent * pressureWeight * errorWeight;
...
float PressureController::getPumpDutyCycleForPressure() {
    if (*_rawPressureSetpoint < 0.2f) {
        ...
        if (_ctrlOutput != nullptr) *_ctrlOutput = 0.0f;
    ...
    if (P < _maxPressure) pressureRatio = P / _maxPressure;
    ...
    float ffDynRawPct = getPressureFeedforwardDynamicUnfiltered(P_ref, _filteredSetpointDerivative, absError);
    ...
    const float maxDropPerCycle = _maxPressureOutputDropRate * _dt;
    ...
    if (!blockIntegral) _errorIntegral = integralCandidate;
